...
func TestSectionCommandsRegisteredWithExpectedFlags(t *testing.T) {
	cmd, _, err := rootCmd.Find([]string{"section", "get"})
	if err != nil {
		t.Fatal(err)
	}
	if cmd != sectionGetCmd {
		t.Fatalf("discovered command = %q, want section get", cmd.CommandPath())
	}
	if cmd.Use != "get <gid>" || cmd.Short != "Fetch a section" {
		t.Errorf("get help metadata = Use %q, Short %q", cmd.Use, cmd.Short)
	}
	for _, example := range []string{
		"dharma section get 123",
		"dharma section get 123 --fields name,project.name",
	} {
		if !strings.Contains(cmd.Example, example) {
			t.Errorf("examples %q do not contain %q", cmd.Example, example)
		}
	}
	fields := cmd.Flags().Lookup("fields")
	if fields == nil {
		t.Fatal("section get --fields is not registered")
	}
	if fields.DefValue != "name" {
		t.Errorf("section get --fields default = %q, want name", fields.DefValue)
	}
	if !strings.Contains(fields.Usage, `--fields ""`) {
		t.Errorf("section get --fields help = %q, want empty-fields escape hatch", fields.Usage)
	}

	listCmd, _, err := rootCmd.Find([]string{"section", "list"})
	if err != nil {
		t.Fatal(err)
	}
	for _, name := range []string{"fields", "project", "paginate"} {
		if listCmd.Flags().Lookup(name) == nil {
			t.Errorf("section list --%s is not registered", name)
		}
	}
	if got := listCmd.Flags().Lookup("fields").DefValue; got != "name" {
		t.Errorf("section list --fields default = %q, want name", got)
	}
}
